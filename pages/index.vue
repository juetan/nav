  ...
  <div class="flex-1 h-full grid grid-rows-[auto_1fr]">
    <div class="">
      <div class="px-6 pb-2 md:pb-1 mx-auto max-w-[1180px] mt-2">
        <div class="bg-gradient-to-br from-blue-300 via-indigo-300 to-blue-300 dark:from-slate-800 dark:to-slate-700 py-8 px-4 dark:bg-gray-800 rounded">
          <div class="text-center">
            <div class="text-xl">前端导航</div>
            <p class="p-0 mt-2.5 text-slate-600">
              涵盖前端开发、设计、运维、测试、产品、运营等领域，目前共收录
              {{ items.length }} 个网站。
            </p>
          </div>
      ...
      <div class="max-w-[1180px] mx-auto mt-6">
        <div v-for="category in showData" :key="category.label" ref="itemsRef" class="fade-in-bottom">
          ...
          <a-empty v-else class="mt-32">
            <template #image>
              <img src="~/assets/svg/empty.svg" alt="empty" class="!h-48" />
            </template>
            结果为空，换个关键字试试？
          </a-empty>
        </div>
...
import { normalData, items } from "~/assets/api";
import tagmaps from "~/assets/api/tags/tag-map.json";
